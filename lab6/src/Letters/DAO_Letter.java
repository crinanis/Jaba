...
public class DAO_Letter implements IGenDAO<Letter, Integer> {
    public final Connection connection;
    public  DAO_Letter(Connection connection){
    ...
    public List <Letter> receiversWithTopic() throws SQLException {
        String sql = "SELECT * FROM letter WHERE topic = \"KGIG\";";
        PreparedStatement preparedStatement = connection.prepareStatement(sql);
        ResultSet resultSet = preparedStatement.executeQuery();
        List<Letter> list = new ArrayList<>();
    ...
    public List <Letter> receiversWithoutTopic() throws SQLException {
        String sql = "SELECT DISTINCT * FROM letter WHERE receiver  NOT IN (SELECT receiver FROM letter WHERE topic = \"KGIG\");";
        PreparedStatement preparedStatement = connection.prepareStatement(sql);
        ResultSet resultSet = preparedStatement.executeQuery();
        List<Letter> list = new ArrayList<>();
        ResulSetListLetter(resultSet, list);
    ...
    public void pointLetter(String Name, String Topic) throws SQLException {
        ...
        var st = connection.prepareStatement("INSERT INTO letter (receiver, sender, topic, `text`, dateOfSend) VALUES(?, ?, ?, ?, '2022-05-23')");
    ...
    public void ResulSetListLetter(ResultSet resultSet, List<Letter> list) throws SQLException {
        while (resultSet.next()) {
            Letter letter = new Letter();
            letter.idLetters = resultSet.getInt("idLetters");
            letter.sender = resultSet.getString("sender");
